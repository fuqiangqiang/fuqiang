...
$(function() {
  ...
  $("body")
    ...
    //删除图片
    .on("tap", ".image-close-exist", function() {
    ...
    mui('body #popover').off().on('tap', '.mui-table-view-cell', function() {
      ...
      if($(this).is('.j-captureImage')) {
        ...
        plus.camera.getCamera().captureImage(function(e) {
          ...
          $("#JD").val(parseFloat(localStorage.getItem("sjzpGeolocationJD")).toFixed(6));
          $("#WD").val(parseFloat(localStorage.getItem("sjzpGeolocationWD")).toFixed(6));
        }, function(e) {
          mui.toast('取消拍照');
        }, {});
        //在点击后关闭 遮罩
        ...
        if(typeof $('#' + idx).attr("imgname") != "undefined") {
          if($('#' + idx).attr("imgname") === idx) {
            mui.each(val, function(index, value) {
              $('#' + idx).append('<div class="imagesBox"><span id="' + value.id + '" class="image-close-exist">×</span>' +
              ...
              if($("input[name='" + idx + "']").hasClass("wind-content-input-choose")) {
                ...
                $.each(typecustom, function(index2, value2) {
                  if(value.type == value2.type && value.value == value2.idnum) {
                    ...
                    $('input[name=' + value2.linktype + ']').siblings('label').removeClass('label-required');
                  }
                }) //--END 关联必填项 --
